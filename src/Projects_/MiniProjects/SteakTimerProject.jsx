...
const SteakTimer = () => {
    ...
    const timers = [rareTimer, mediumRareTimer, mediumTimer, mediumWellTimer, wellTimer];
    const images = [rare, mediumRare, medium, mediumWell, wellDone]
    const steakType=['Rare', 'Medium Rare', 'Medium', 'Medium Well', 'Well Done']
    const steakDashboard = steakType.map((title, index) => (
        <button key={index} className="text-white bg-slate-700 p-5 text-xl mt-30 rounded-2xl" onClick={timers[index]}>
    ...
    return (
        <>
            <div id="steak-bg">
                <button><Link to='/mini-projects'>Back</Link></button>
